...
from NSGPy.torch.model import NSGP as NST
...
np.random.seed(seed)
def rand(shape): return np.abs(np.random.normal(loc=0, scale=1, size=shape))
...
def f_indu(x, num_ind): return KMeans(n_clusters=num_ind,
...
vs.positive(init=local_gp_noise_std, shape=(
    input_dim,), name='local_gp_noise_std')
...
model = NST(torch.tensor(X), torch.tensor(
    y), num_inducing_points, f_indu, params, seed)
