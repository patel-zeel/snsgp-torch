...
class NSGPRegression:
    ...
    def init_params(self, seed):
        ...
        # Local params
        self.vs.positive(init=np.random.normal(), name='local_std')
        ...
        # Global params
        self.vs.positive(init=np.random.normal(), name='global_std')
        self.vs.positive(init=np.random.normal(), name='global_noise_std')
    ...
    def LocalGP(self, vs, X, return_logpdf=False): # Getting lengthscales for entire train_X (self.X)
        ...
        for dim in range(self.input_dim):
            f = GP(vs['local_std']**2 * EQ().stretch(vs['local_gp_ls'][dim]))
            f_post = f | (f(self.X_bar[:, dim], vs['local_gp_noise_std'][dim]**2), vs['local_ls'][dim])
            l = f_post(X[:, dim], vs['local_gp_noise_std'][dim]**2).mean.mat
            l_list.append(l)
    ...
    def GlobalGP(self, vs): # Construct global GP and return nlml
        ...
        f = GP(vs['global_std']**2 * NSEQ(global_ls, global_ls))
        
        return -f(self.X, vs['global_noise_std']**2).logpdf(self.y) - tf.reduce_sum(log_pdf_list)
    ...
    def predict(self, X_new): # Predict at new locations
        ...
        K = self.vs['global_std']**2 * NSEQ(global_ls, global_ls)(self.X, self.X)
        K_star = self.vs['global_std']**2 * NSEQ(global_ls_new, global_ls)(X_new, self.X)
        K_star_star = self.vs['global_std']**2 * NSEQ(global_ls_new, global_ls_new)(X_new, X_new)
        
        L = B.cholesky(K + B.eye(self.X.shape[0]) * self.vs['global_noise_std']**2)
        alpha = B.cholesky_solve(L, self.y)
        
        pred_mean = K_star@alpha
        
        v = B.cholesky_solve(L, B.T(K_star))
        pred_var = K_star_star + B.eye(X_new.shape[0])*self.vs['global_noise_std']**2 - K_star@v
