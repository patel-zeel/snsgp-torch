...
class NSGPRegression:
    ...
    random_state: int, Random number to reproduce the results
    
    """
    def __init__(self, input_dim, num_inducing_points=5,
                 random_state=0, ARD=True):
    ...
    def K_bar(self, sigma_f_bar, sigma_l_bar, X_bar_1, X_bar_2=None): # RBF kernel for lengthscale GP for training
        ...
        X_bar_1, X_bar_2: np.ndarray with shape (num_inducing_points, input_dim)
        sigma_f, sigma_n: int, signal standard deviation and noise standard deviation
        """
        if X_bar_2 is None:
            X_bar_2 = X_bar_1
        ...
        for dim in range(self.input_dim): # Oops, No vectorization available
            if self.l_isotropic:
                if self.optimizer == 'jax':
                    ...
                    self.K__X_bar__X_bar.ravel()[dim::p*r+r] += sigma_n_bar**2 # Add noise variance to diagonal
                else:
                ...
                elif self.optimizer in ['scipy', 'lsq']:
                    self.K__X_bar__X_bar.ravel()[dim::p*r+r] += sigma_n_bar[dim]**2 # Add noise variance to diagonal
                else:
                    raise NotImplementedError('LLS: Optimizer: '+self.optimizer+' is not implemented')
    ...
    def mll(self, params): # Marginal log likelihood from equation 6 in section 4.1 
        ...
        self.K_XX.ravel()[::p+1] += sigma_n**2 # adding noise variance to diagonal
    ...
    def lsq(self, params):
        ...
        Kernel = self.K(sigma_f, self.X, l)
        p = Kernel.shape[0]
        Kernel.ravel()[::p+1] += sigma_n**2
        return np.mean(np.square(Kernel.ravel() - self.cov.ravel()))
...
#                           zind
                    bounds[2:2+self.input_dim] = [(0.9, 1.1) for _ in range(self.input_dim)] #fixed
#                         bounds[2+self.input_dim*2:2+self.input_dim*3] = [(10**-5, 10**-4) for _ in range(self.input_dim)]
#                         bounds[2+self.input_dim*3:] = [(0.01, 100) for _ in range(len(bounds[2+self.input_dim*3:]))]
                ...
                if self.optimizer=='scipy':
                    ...
                    res = scipy.optimize.minimize(self.lsq, params, bounds=bounds) #method=self.method)
...
#         print(self.l_bar, l)
    ...
    def predict(self, X_star, return_cov=True, diag=True): # Predicting output
        ...
        p,_ = K_XX.shape
        K_XX.ravel()[::p+1] += self.sigma_n**2 # adding noise variance to diagonal
        L = np.linalg.cholesky(K_XX)
        ...
        if return_cov:
            ...
            if diag: # return only diagonal entries of covariance
                cov = self.K_(X_star, l_star, diag=True)+np.ones(p_star)*self.sigma_n**2 - (K__X_star__X@v).diagonal()
            else:
                cov = self.K_(X_star, l_star)+np.eye(p_star)*self.sigma_n**2 - K__X_star__X@v
            return mean, cov
        return mean